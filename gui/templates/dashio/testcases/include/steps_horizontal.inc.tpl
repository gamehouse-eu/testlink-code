...
*}
  ...
  <tr id="step_row_{$step_info.step_number}">
    ...
    <td {if $edit_enabled && $args_frozen_version=="no"} style="cursor: pointer; padding-left: 5px;" onclick="launchEditStep({$step_info.id})" {/if}>{if $gui->stepDesignEditorType == 'none'}{$step_info.actions|nl2br}{else}{$step_info.actions}{/if}
    {if $ghost_control}<p class="ghost" style="display: none;">{$step_info.ghost_action}</p>{/if}
    </td>
    <td {if $edit_enabled && $args_frozen_version=="no"} style="cursor: pointer; padding-left: 5px;" onclick="launchEditStep({$step_info.id})" {/if}>{if $gui->stepDesignEditorType == 'none'}{$step_info.expected_results|nl2br}{else}{$step_info.expected_results}{/if}
    {if $ghost_control}<p class="ghost" style="display: none;">{$step_info.ghost_result}</p>{/if}
    </td>
    {if $gui->tprojOpt->automationEnabled}
    <td {if $edit_enabled && $args_frozen_version=="no"} style="cursor: pointer; text-align: center;" onclick="launchEditStep({$step_info.id})" {/if}>{$gui->execution_types[$step_info.execution_type]}</td>
    {/if}
